...
export class TokenManager {
  ...
  async refreshTokenIfNeeded(
    ...
    try {
      ...
      // Google (and some other providers) only return access_token on refresh — the
      // refresh_token stays the same and is omitted from the response. Keep the
      // existing encrypted refresh in DB unless the platform sends a new one.
      ...
      if (newTokens.refreshToken) {
        const encryptedRefresh = this.encryptToken(newTokens.refreshToken, account.id)
        payload.refresh_token = encryptedRefresh.encrypted
        payload.refresh_token_iv = encryptedRefresh.iv
        payload.refresh_token_tag = encryptedRefresh.tag
      }

      await supabaseAdmin.from("ad_accounts").update(payload).eq("id", account.id)
